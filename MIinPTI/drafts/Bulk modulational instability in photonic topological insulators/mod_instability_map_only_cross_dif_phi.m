...
ind_phi       
        ...
        for d=1:1:length(Delta)
                    ...
                    if (I_0/2>=-M/g) && (I_0/2>=M/g)
                        psi_1=sqrt(I_0/2+M/g).*exp(1i*phi(ind_phi));
                        psi_2=sqrt(I_0/2-M/g);
                        syms Omega x
                        kappa_x= [-2:0.1:2];
                    parfor ix = 1:1:length(kappa_x) %для каждого каппа составляем матрицу
                                Mat=[-Omega-omega+(M+beta*kappa_x(ix)^2) - 2*g*abs((psi_1)^2),-g*psi_1^2,- sqrt(2).*J_1.*kappa_x(ix),0;
                                - sqrt(2).*J_1.*kappa_x(ix),0,-Omega - omega  - (M+beta*kappa_x(ix)^2) - 2*g*abs(psi_2^2), - g*psi_2^2; 
                                -g*conj(psi_1^2),Omega - omega + (M+beta*kappa_x(ix)^2) - 2*g*abs(psi_1^2),0,sqrt(2).*J_1.*kappa_x(ix); %%
                                0,sqrt(2).*J_1.*kappa_x(ix),-g*conj(psi_2^2),Omega - omega - (M+beta*kappa_x(ix)^2) - 2*g*abs(psi_2^2)];%%
                                eqn = det(Mat) == 0; 
        ...
        p(1,ind_phi).select();
        title(['The instability of the cross point for $\varphi$ = ',num2str(phi(ind_phi))])
